...
//
// It's OK to call InfoForRegistry concurrently.
type Config interface {
	// InfoForRegistry returns auth information for the given host.
	// If there's no information available, it should return the zero ConfigInfo
	// and nil.
	InfoForRegistry(host string) (ConfigInfo, error)
}

// ConfigInfo holds auth information for a registry.
...
// and no error.
type HelperRunner = func(helperName string, serverURL string) (ConfigInfo, error)
...
// InfoForRegistry implements [Authorizer.InfoForRegistry].
// If no registry is found, it returns the zero [ConfigInfo] and a nil error.
func (c *ConfigFile) InfoForRegistry(registryHostname string) (ConfigInfo, error) {
	...
	if auth.IdentityToken != "" && auth.Username != "" {
		return ConfigInfo{}, fmt.Errorf("ambiguous auth credentials")
	}
	if len(auth.derivedFrom) > 1 {
		return ConfigInfo{}, fmt.Errorf("more than one auths entry for %q (%s)", registryHostname, strings.Join(auth.derivedFrom, ", "))
	}
...
func ExecHelper(helperName string, serverURL string) (ConfigInfo, error) {
	...
	if err := cmd.Run(); err != nil {
		if !errors.As(err, new(*exec.ExitError)) {
			return ConfigInfo{}, fmt.Errorf("cannot run auth helper: %v", err)
		}
		t := strings.TrimSpace(out.String())
		if t == "credentials not found in native keychain" {
			return ConfigInfo{}, nil
		}
		return ConfigInfo{}, fmt.Errorf("error getting credentials: %s", t)
	}
