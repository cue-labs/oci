...
type ResourceScope struct {
	...
	// Known values for this include "registry" and "repository".
	...
	// For resource type "repository", this will be the name of the repository.
	Resource string

	// Action names an action that can be performed on the resource.
	// This is usually "push" or "pull".
	Action string
}
...
// Scope holds a set of authorization scopes. The zero value
...
func (s Scope) String() string {
	...
	s.Iter()(func(s ResourceScope) bool {
		prev0 := prev
		prev = s
		if s.ResourceType == "repository" && prev0.ResourceType == "repository" && s.Resource == prev0.Resource {
			buf.WriteByte(',')
