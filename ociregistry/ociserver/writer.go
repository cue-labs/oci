...
func (r *registry) handleBlobUploadChunk(ctx context.Context, resp http.ResponseWriter, req *http.Request, rreq *ocirequest.Request) error {
	...
	start, _, size, err := chunkRangeAndSize(req)
	if err != nil {
		return err
	}

	w, err := r.backend.PushBlobChunkedResume(ctx, rreq.Repo, rreq.UploadID, start, size)
	if err != nil {
		return err
	}
...
func chunkRangeAndSize(req *http.Request) (start, end int64, size int, _ error) {
	var rangeOK bool
	if s := req.Header.Get("Content-Range"); s != "" {
		start, end, rangeOK = ocirequest.ParseRange(s)
		if !rangeOK {
			return 0, 0, 0, badAPIUseError("we don't understand your Content-Range")
		}
	}
	...
	if rangeOK {
		size = int(end - start)
	} else if s := req.Header.Get("Content-Length"); s != "" {
		var err error
		size, err = strconv.Atoi(s)
		if err != nil {
			return 0, 0, 0, badAPIUseError("we don't understand your Content-Length")
		}
	}
