	...
	return func(yield func(T, error) bool) {
		...
		for {
			...
			for _, item := range items {
				...
				if canUseLast && initialReq.ListLast != "" && fmt.Sprint(item) <= initialReq.ListLast {
					continue
				}
			...
			if len(items) > initialReq.ListN {
				...
				// most notably Google Container/Artifact Registry.
				//
				// This would already be sufficiently handled by the workaround in the loop above,
				// but this check avoids an extra request that would just result in 0 items (after our filtering).
				//
				// If we receive more items than we should have, assume the registry does not implement pagination,
				// and there is no point in doing further requests.
				return
			}
